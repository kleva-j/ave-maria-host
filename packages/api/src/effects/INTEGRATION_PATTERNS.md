...
app.post(
  ...
  effectMiddlewareWithContext((context) =>
    Effect.gen(function* (_) {
      ...
      // Parse request body
      const body = yield* _(
        parseJSON<CreateUserData>(await c.req.text(), "requestBody")
      );
      ...
      const hashedPassword = yield* _(
        auth.hashPassword(validatedData.password)
      );
...
## @effect/rpc Integration Patterns

### Native Effect RPC Implementation

The application now uses native @effect/rpc instead of custom oRPC + Effect adapters.

```typescript
import { Rpc, RpcGroup } from "@effect/rpc";
import { Effect, Layer } from "effect";

// Create native Effect RPC group
...
### Client-Side @effect/rpc Integration

```typescript
// Client-side usage with native Effect RPC
...
class UserRepository {
  ...
  async create(userData: CreateUserData): Promise<User> {
    try {
      await db.execute("INSERT INTO users (id, email, name) VALUES (?, ?, ?)", [
        userData.id,
        userData.email,
        userData.name,
      ]);
...
const UserService = {
  ...
  findByEmail: (email: string) =>
    Effect.gen(function* (_) {
      ...
      const user = yield* _(
        db.queryOne<User>("SELECT * FROM users WHERE email = ?", [email]).pipe(
          Effect.catchAll(() =>
  ...
  return (...args: TArgs): Effect.Effect<TReturn, ApplicationError> => {
    ...
    return fromPromise(
      ...
      (error) =>
        new NetworkError({
          message:
            error instanceof Error ? error.message : "Migration wrapper error",
...
describe("UserService", () => {
  describe("findById", () => {
    it("should return user when found", async () => {
      const result = await runTest(UserService.findById("existing-user"));
    ...
    it("should return null when user not found", async () => {
      const result = await runTest(UserService.findById("non-existent-user"));
  ...
  describe("createUser", () => {
    it("should create user successfully", async () => {
      ...
      const result = await runTest(UserService.createUser(userData));
    ...
    it("should handle validation errors", async () => {
      ...
      const result = await runTestEither(UserService.createUser(userData));
